...
class BIP39 {
	 ...
	 */
	public static function generateEntropy($size = 256): string {
		if ($size % 32 !== 0) {
	 ...
	 * @param array $wordList       defaults to BIP39 english word list
	 * @return string                       hex Mnemonic
	 * @throws \Exception
	 */
	public static function entropyToMnemonic($entropyHex, array $wordList = null): string {
	 ...
	 */
	protected static function entropyChecksum($entropyHex): string {
		// calculate entropy, /2 because PHP can't do bytes
		$ENT = (strlen($entropyHex) / 2) * 8;
		// calculate how long the checksum should be
		$CS = $ENT / 32;
	 ...
	 * @throws \Exception
	 */
	public static function mnemonicToEntropy($mnemonic, array $wordList = null): string {
		...
		foreach ($words as $word) {;
			$idx = array_search($word, $wordList);
	 ...
	 * @throws \Exception
	 */
	public static function mnemonicToSeedHex($mnemonic, $passphrase): string {
	 ...
	 * @throws \Exception
	 */
	public static function normalizePassphrase($passphrase): string {
